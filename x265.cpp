...
int main(int argc, char **argv)
{
    ...
    if (cliopt.parse(argc, argv, param)) //参数读取
    {
        cliopt.destroy();
        exit(1);
    }

    x265_encoder *encoder = x265_encoder_open(param); //编码初始信息显示，全局变量赋值，编码器初始化...
    if (!encoder)
    {
    ...
    x265_picture_init(param, pic_in); //pic_in初始化
    ...
    while (pic_in && !b_ctrl_c)
    {
        ...
        if (cliopt.framesToBeEncoded && inFrameCount >= cliopt.framesToBeEncoded)
            pic_in = NULL;
        else if (cliopt.input->readPicture(pic_orig)) //读入图像参数信息
            inFrameCount++;
        else
            pic_in = NULL;
		...
		//编码函数，返回值表示编码成功与否(-1:退出,0:未编完...)
		//encoder:编码器, p_nal:bit流,nal:nal个数,pic_in:读入图像,pic_recon:重建图像
		//nal是什么?
        ...
        {
            cliopt.recon->writePicture(pic_out); //重建图像(如何重建?)
        }
    ...
    while (!b_ctrl_c)
    {
        ...
        if (nal)
            cliopt.writeNALs(p_nal, nal); //bit流输出
