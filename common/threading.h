...
typedef pthread_t ThreadHandle; //线程ID

class Lock //互斥锁，防止race condition
{
public:
 ...
 * the writer thread are visible prior to readers seeing the m_val change. Its
 * secondary purpose is for use with the condition variable for blocking waits */
class ThreadSafeInteger //线程安全的整数类型
{
public:
    ...
    int waitForChange(int prev) //阻塞直到值改变
    {
        pthread_mutex_lock(&m_mutex);
        if (m_val == prev)
            pthread_cond_wait(&m_cond, &m_mutex);
...
//区域锁。锁的初始化放到构造函数，锁的释放放到析构函数。这样当锁离开作用域时，析构函数会自动释放锁。
//利用作用域自动解锁
class ScopedLock 
{
public:
