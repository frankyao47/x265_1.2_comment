...
x265_encoder *x265_encoder_open(x265_param *p)
{
    ...
    if (x265_check_params(param)) //检查参数是否合法
        return NULL;

    if (x265_set_globals(param)) //全局变量赋值
        return NULL;

    Encoder *encoder = new Encoder; //编码器初始化
    if (encoder)
    {
        ...
        x265_print_params(param); //编码初始阶段的显示信息
        encoder->create();
        encoder->init();
    }
...
int x265_encoder_encode(x265_encoder *enc, x265_nal **pp_nal, uint32_t *pi_nal, x265_picture *pic_in, x265_picture *pic_out)
{
    ...
    do
    {
        numEncoded = encoder->encode(pic_in, pic_out); //编码环节
    }
    while (numEncoded == 0 && !pic_in && encoder->m_numDelayedPic);

    if (pp_nal && numEncoded > 0)
    {
        *pp_nal = &encoder->m_nalList.m_nal[0]; //m_nalList.m_nal[0]写入p_nal
        if (pi_nal) *pi_nal = encoder->m_nalList.m_numNal; //m_numNal写入nal
    }
    else if (pi_nal)
        *pi_nal = 0;
