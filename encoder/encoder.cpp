...
void Encoder::create()
{
    ...
    if (m_frameEncoder)
    {
        ...
        {
            m_frameEncoder[i].setThreadPool(m_threadPool); //不同线程属于同一个线程池内
        }
    }
...
int Encoder::encode(const x265_picture* pic_in, x265_picture *pic_out)
{
    ...
    if (pic_in)
    {
        ...
        {
            pic = new Frame; 
            if (!pic || !pic->create(m_param, m_defaultDisplayWindow, m_conformanceWindow)) //Frame初始化
            {
                m_aborted = true;
        ...
        pic->m_POC = ++m_pocLast;
        pic->reinit(m_param);
        pic->getPicYuvOrg()->copyFromPicture(*pic_in, m_pad); //像素点赋值,这部分也是帧初始化过程
    ...
    m_curEncoder = (m_curEncoder + 1) % m_param->frameNumThreads; //当前encoder,与线程数有关
    int ret = 0;
