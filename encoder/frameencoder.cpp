...
void FrameEncoder::compressFrame()
{
    ...
    TEncEntropy* entropyCoder      = getEntropyCoder(0); //熵编码对象(来自m_rows[0].m_entropyCoder)
    TComSlice*   slice             = m_frame->getSlice();
    int          totalCoded        = m_rce.encodeOrder;

    entropyCoder->setEntropyCoder(&m_sbacCoder, NULL); //熵编码

    /* Emit access unit delimiter unless this is the first frame and the user is
     * not repeating headers (since AUD is supposed to be the first NAL in the access
     * unit) */
     //access unit delimiter:帧分界符
    if (m_param->bEnableAccessUnitDelimiters && (m_frame->getPOC() || m_param->bRepeatHeaders))
    {
        m_bs.resetBits();
        entropyCoder->setBitstream(&m_bs);
        entropyCoder->encodeAUD(slice); //Access unit delimiter
...
void FrameEncoder::compressCTURows()
{
    ...
    {
        m_rows[i].init(slice);
        m_rows[i].m_entropyCoder.setEntropyCoder(&m_sbacCoder, slice); //熵编码器获取slice, qp
        m_rows[i].m_entropyCoder.resetEntropy(); //slice初始化
